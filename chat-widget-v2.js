...
`;
  ...
  /* ─── Send message ───────────────────────────────────────────── */
  async function sendMessage(text, cta) {
    quickArea.innerHTML = '';
    addUserMsg(text);
    ...
    try {
      ...
      // Determine CTA: passed directly from CTA button, or keyword match for services
      let ctaToShow = cta || null;
      if (!ctaToShow) {
        const serviceKeywords = ['service', 'services', 'offer', 'what do you do'];
        const isServiceQ = serviceKeywords.some(kw => text.toLowerCase().includes(kw));
        if (isServiceQ) {
          const matchedQ = (cfg().branding?.quickQuestions || [])
            .map(q => typeof q === 'string' ? null : q)
            .find(q => q && q.cta && q.text.toLowerCase().includes('service'));
          if (matchedQ) ctaToShow = matchedQ.cta;
        }
      }
